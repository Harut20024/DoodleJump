...
let platforms = [];
let score = 0;
let doodleLeftImg, doodleRigthImg, doodleJumpImg, platformImg, monsterImg1, monsterImg2, jetImg, doodleInJet;
let backgraundImg;
let speed = 5;
let jumpCount = 0;
let startY
...
function draw() {
    if (!gameStarted) {
        ...
        while (platforms.length < 6 || platforms[platforms.length - 1].y > -transY) {
            ...
            if (randNum === 1) {
                // Add a platform with a monster
                platforms.push(new Platform(random(width - 90), newPlatformY, platformImg, true, MonsterArr[randNumforMonster], false));
            } else if (randNumJet === 1 && randNum !== 1) {
                // Add a platform with a jet
                platforms.push(new Platform(random(width - 90), newPlatformY, platformImg, false, null, true));
            } else {
                // Add a regular platform without monster or jet
                platforms.push(new Platform(random(width - 90), newPlatformY, platformImg, false, null, false));
            }
        }
    ...
    your score is ${score}`
        ...
        if (keyIsDown(32) && currentTime - lastShotTime > 1000 && !doodler.isFlying) {
            if (!shootSound.isPlaying()) shootSound.play();
            lastShotTime = currentTime;
            let newBall = new Ball(doodler.position.x, doodler.position.y, goingLeft, isJumping);
            balls.push(newBall);
        }
        ...
        for (let i = balls.length - 1; i >= 0; i--) {
            ...
            if (balls[i] && (balls[i].position.y < -transY || balls[i].position.x > width || balls[i].position.x < 0)) {
                balls.splice(i, 1);
            }
...
function isColliding(ball, platform) {
    let withinXBounds = ball.position.x > platform.x && ball.position.x < platform.x + platform.width;
    let withinYBounds = ball.position.y + ball.size / 2 + 50 > platform.y && ball.position.y - ball.size / 2 - 50 < platform.y + platform.height;
